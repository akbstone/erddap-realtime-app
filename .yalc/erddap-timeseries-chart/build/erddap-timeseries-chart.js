(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3-array'), require('d3-shape'), require('d3-axis'), require('d3-scale'), require('d3-selection'), require('d3-dispatch'), require('d3-color'), require('d3-scale-chromatic')) :
	typeof define === 'function' && define.amd ? define(['exports', 'd3-array', 'd3-shape', 'd3-axis', 'd3-scale', 'd3-selection', 'd3-dispatch', 'd3-color', 'd3-scale-chromatic'], factory) :
...
}(this, function (exports, d3Array, d3Shape, d3Axis, d3Scale, d3Selection, d3Dispatch, d3Color, scaleChromatic) { 'use strict';
	...
	function chart(){
		...
		let x = function(d){return d[0]},
			...
			chartOptions = {},
			rule,
			chart_dispatcher = d3Dispatch.dispatch("mousemove","mouseout"),
		...
		function calculateDomains(){
			...
			if(data){
				if(x){
					domains.xDomain = d3Array.extent(data,x);
				}
				if(y){
					domains.yDomain = d3Array.extent(data,y);
				}
				if(z){
					domains.zDomain = d3Array.extent(data,z);
				}
			}
	  ...
	  chart.curtainScheme = function(colorScheme) {
		...
		function drawCurtain(){
			...
			let	canvas = this.selection().node(),
				...
				xScale = d3Scale.scaleUtc()
							.domain(xDomain)
							.range([0,width]),

				zScale = d3Scale.scaleLinear()
							.domain(zDomain)
							.range([height,0]),
				bisect = d3Array.bisector(d => d).left,
				gpd = d3Array.group(data,d=>String(x(d))),
			...
			for(var gy=0;gy < grid.length;gy ++){
				for(var gx=0;gx < grid[gy].length;gx ++){
					let vals = grid[gy][gx],
						...
						_gx = gx*cell_width,
						c = vals ? d3Color.rgb(interpolateFn(yScale(d3Array.mean(vals)))) : null,
			...
			function mousemove() {
				...
				const selected_x_value = xScale.invert(d3Selection.mouse(this)[0]);
		...
		function drawLine(){
			...
			if(xDomain && yDomain){
	      	let fn = (xDomain[0] instanceof Date) ? d3Scale.scaleUtc : d3Scale.scaleLinear,
	          xScale = fn()
						.domain(xDomain)
	          ...
	          // @TODO: need to add xLabel if exists here
					...
					yAxis = g => g
						...
						.call(d3Axis.axisLeft(yScale).ticks(height / 40).tickSizeOuter(0))
				...
				function mousemove() {
					const bisect = d3Array.bisector(d => x(d)).left;
					const mouse_pos = d3Selection.mouse(this);
